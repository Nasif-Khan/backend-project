...
const logoutUser = asyncHandler(async (req, res) => {
    await User.findByIdAndUpdate(
        ...
        {
            $unset: {
                refreshToken: 1  // remove the refresh token from the user object
            }
        },
        {
            new: true
...
const getUserWatchHistory = asyncHandler(async (req, res) => {
    const user = await User.aggregate([
        {
            $match:{
                // _id = req.user?._id     [ returns string ] ------> not allowed here, mongoose does not work here,
                //  the code in aggregation goes directly , we need object id
